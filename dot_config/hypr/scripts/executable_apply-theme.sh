...
readonly TEMPLATE_PATH="$HOME/Documents/Projects/Python/iroha-templates"
readonly VIMRC_PATH="$XDG_CONFIG_HOME/nvim/lua/extras/lush/iroha/colors/iroha.lua"
...
main() {
    ...
    if "$dark_mode"; then
        ...
        pywalfox dark
        sed -i 's/vim\.opt\.background\s*=\s*"light"/vim.opt.background = "dark"/' "$VIMRC_PATH"
    else
        echo "Applying light theme."
        gsettings set org.gnome.desktop.interface color-scheme 'default'
        pywalfox light
        sed -i 's/vim\.opt\.background\s*=\s*"dark"/vim.opt.background = "light"/' "$VIMRC_PATH"
    fi
    ...
    if [[ "$source_type" == 'image' ]]; then
        ...
        iroha_args+=("from-image" "$source_value" "-q" "30" "-m" "$TEMPLATE_PATH/template_map.yaml")
        ...
        iroha_args+=("from-color" "$source_value" "-m" "$TEMPLATE_PATH/template_map.yaml")
    ...
    if [[ -n "$mouse" ]]; then
        local primary_hex
        primary_hex=$(echo "$json_colour" | jq -r .primary.hex)
    ...
    local primary_rgb
    primary_rgb=$(echo "$json_colour" | jq -r .primary.rgb | sed 's/ //g' | sed 's/,1)/)/g')
    local primary_container_rgb
    primary_container_rgb=$(echo "$json_colour" | jq -r .primaryContainer.rgb | sed 's/ //g' | sed 's/,1)/)/g')
